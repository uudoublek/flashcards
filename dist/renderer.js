  ...
  `;
    ...
    for (const t of topics) {
        ...
        card.addEventListener("click", () => {
            document.dispatchEvent(new CustomEvent("topic-select", { detail: t }));
        });
...
export function renderTopicHome(topic, _newCount, dueCount) {
    clearApp();
  ...
  `;
    ...
    document.getElementById("btn-learn-mode").addEventListener("click", () => {
        document.dispatchEvent(new CustomEvent("start-learn", { detail: { tags: [...selectedTags] } }));
    });
    ...
    document.getElementById("btn-browse-mode").addEventListener("click", () => {
        document.dispatchEvent(new CustomEvent("start-browse", { detail: { tags: [...selectedTags] } }));
    });
}
export function renderLearn(_session, card, cardType, progress) {
    renderCard({
        card,
        cardType,
        mode: "learn",
        progress,
        onRate: (rating) => {
            document.dispatchEvent(new CustomEvent("learn-rate", { detail: { rating } }));
        },
        showFlip: true,
    });
}
// ========== 复习模式 ==========
export function renderReview(card, cardType, progress) {
    renderCard({
        card,
        cardType,
        mode: "review",
        ...
        onRate: (rating) => {
            document.dispatchEvent(new CustomEvent("review-rate", { detail: { rating } }));
        },
        showFlip: true,
    });
}
export function renderBrowse(card, cardType, session) {
    renderCard({
        card,
        cardType,
        mode: "browse",
...
// ========== 完成提示 ==========
export function renderLearnDone(groupName, remainingGroups) {
    clearApp();
...
export function renderReviewDone() {
    clearApp();
    const container = el("div", "done-screen");
    container.innerHTML = `
    <div class="done-icon">🎉</div>
...
// ========== 浏览列表 ==========
export function renderBrowseList(topic, selectedTags, selectedTypeIds) {
    clearApp();
    const container = el("div", "browse-list");
    const tagSet = new Set(selectedTags);
    const filtered = topic.cards.filter(c => c.tags.some(t => tagSet.has(t)));
    container.innerHTML = `
  ...
  `;
    ...
    const cardTypes = topic.meta.cardTypes.filter(ct => selectedTypeIds.includes(ct.id));
    for (const card of filtered) {
        for (const ct of cardTypes) {
            const item = el("div", "browse-item");
            const front = ct.front.map(f => card.fields[f] ?? "").join(" ");
            // strip HTML for preview
            const strip = (s) => s.replace(/<[^>]*>/g, "").slice(0, 60);
            item.innerHTML = `
      ...
      `;
            item.addEventListener("click", () => {
                document.dispatchEvent(new CustomEvent("browse-card", {
...
// ========== 标签选择缓存 ==========
function getSelectedTags(topicId) {
    try {
...
function setSelectedTags(topicId, tags) {
